...
function tidyUpString(str) {
  return str.map(x => x.split("/").join(" ").trim().toLowerCase());
}
...
function remove(newArr, index) {
  let copyArr = newArr.slice();
  let value = copyArr.splice(index, 1);
  return copyArr.filter((el) => el !== value);
}
...
function formatPercentage(percent) {
  return percent.map((elt) =>
    elt < 100 && typeof elt === "number"? `${parseFloat(elt.toFixed(2))}%` : "100%"
  );
}
